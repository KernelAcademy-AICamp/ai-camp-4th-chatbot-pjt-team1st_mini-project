...
### 1.2 프로젝트 목적
국립중앙박물관 공식 앱 내 기존 안내 챗봇에 **학습 기능을 추가**하여, 관람객이 전시 유물을 보다 깊이 이해하고 능동적으로 학습할 수 있도록 돕는 **인터랙티브 학습 파트너** 서비스를 구축한다.
...
#### 기존 앱 챗봇의 한계
현재 국립중앙박물관 공식 앱에서 제공하는 챗봇은 다음과 같은 **안내 기능**에 집중되어 있다:
- 관람 정보 안내 (운영 시간, 위치 등)
- 전시 일정 및 구성 안내
- 오시는 길 등 기본 FAQ 제공

이러한 안내 기능만으로는 관람객의 **학습 욕구**를 충족시키기 어렵다.
...
#### 단기 목표 (MVP)
- 기존 안내 챗봇에 통합 가능한 **학습 기능 모듈** 프로토타입 구현
- 국보급 유물 15점 기반 퀴즈 학습 시스템 구축
- AI 기반 동적 퀴즈 생성 및 맞춤형 해설 제공

#### 중장기 목표
- 국립중앙박물관 공식 앱 내 기존 챗봇 시스템에 학습 기능 통합
- 전체 소장품 데이터베이스 연동
- 연령대별 맞춤 학습 콘텐츠 확대
...
|-----|------|
| **기능명** | 학습할 유물 선택 |
| **목적** | 방금 관람한 유물 중 학습하고 싶은 유물을 직접 선택 |
| **입력** | 사용자의 유물 선택 (터치/클릭) |
| **출력** | 선택된 유물 목록 (3~10개) |
...
│  │  ┌─────────────────────────────────────────────────────────────┐   │   │
│  │  │                    Session State (메모리)                    │   │   │
│  │  │  • selected_artifacts    • generated_quizzes                │   │   │
│  │  │  • current_quiz_index    • score, answers                   │   │   │
│  │  │  • chat_history          • stage (select/quiz/result)       │   │   │
│  │  └─────────────────────────────────────────────────────────────┘   │   │
│  └───────────┬──────────────────────────────────┬──────────────────────┘   │
│              │                                  │                          │
│   ② 유물 데이터 요청                    ③ 퀴즈/해설 생성 요청                │
│              ▼                                  ▼                          │
│  ┌─────────────────────┐            ┌─────────────────────┐               │
│  │   국립중앙박물관 API  │            │   Google Gemini API  │               │
│  │   (e-Museum)        │            │   (AI 생성)          │               │
│  ├─────────────────────┤            ├─────────────────────┤               │
│  │ • 유물 목록 조회     │            │ • 동적 퀴즈 생성     │               │
│  │ • 유물 상세 정보     │            │ • 맞춤형 해설 생성   │               │
...
```
┌─────────┐      ┌──────────────┐      ┌─────────────┐      ┌──────────────┐
│ 앱 시작 │ ───▶ │ artifacts.py │ ───▶ │ 유물 15점   │ ───▶ │ 카드 UI 표시 │
└─────────┘      │ (로컬 DB)    │      │ 데이터 로드  │      └──────┬───────┘
                 └──────────────┘      └─────────────┘             │
                                                                   ▼
┌──────────────┐      ┌─────────────────┐      ┌─────────────────────────┐
│ 퀴즈 시작    │ ◀─── │ selected_ids    │ ◀─── │ 사용자 유물 선택 (3-10) │
│ 버튼 활성화  │      │ (Set 자료구조)  │      └─────────────────────────┘
└──────────────┘      └─────────────────┘
```
...
```
┌─────────────────┐      ┌───────────────────────────────────────────────┐
│ selected_       │      │                  Gemini API                   │
│ artifacts[]     │ ───▶ │  Prompt: "다음 유물에 대한 퀴즈를 생성해주세요" │
│ (선택된 유물)    │      │  + 유물명, 시대, 재질, 설명                    │
└─────────────────┘      └───────────────────────┬───────────────────────┘
                                                 │
                                                 ▼
                         ...
                         │  {                                            │
                         │    "NMK-001": {                               │
                         │      "question": "이 유물의 제작 시기는?",      │
                         │      "options": ["삼국", "고려", "조선", "근대"],│
                         │      "answer": 0,                             │
                         │      "explanation": "삼국시대 6세기..."        │
                         │    },                                         │
                         │    ...                                        │
                         │  }                                            │
                         └───────────────────────────────────────────────┘
...
│  [                                                                        │
│    {                                                                      │
│      "artifact": "금동미륵보살반가사유상",                                   │
│      "question": "이 불상의 자세를 무엇이라고 하나요?",                       │
│      "user_answer": "반가사유자세",                                         │
│      "correct_answer": "반가사유자세",                                      │
│      "is_correct": true,                                                  │
│      "user_question": "반가사유자세의 의미는?",                              │
│      "explanation": "정답입니다! 반가사유자세는..."                          │
│    },                                                                     │
│    ...                                                                    │
│  ]                                                                        │
└───────────────────────────────────────────────────────────────────────────┘
...
| 단계 | 입력 데이터 | 처리 | 출력 데이터 | 저장 위치 |
|-----|-----------|------|-----------|----------|
| 시작 | - | 유물 DB 로드 | 유물 15점 | Session State |
| 선택 | 사용자 터치 | ID 수집 | selected_ids | Session State |
| 퀴즈 생성 | 유물 정보 | Gemini API 호출 | 퀴즈 JSON | Session State |
| 퀴즈 풀이 | 답안 + 질문 | 정답 체크 + AI 해설 | 응답 기록 | Session State |
| 결과 | answers[] | 점수 집계 | 점수, 리뷰 | Session State |
...
| **Phase 7** | 배포 및 앱 통합 | 웹 배포, 기존 앱 연동 |
